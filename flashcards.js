                ...
                inquirer.prompt([
                  ...
                  //once the user gives answer....
                    if (appData.anotherCard === "Yes") {
                      //If 'Yes' then..
                        createCard();
                      //call the create card function again (recursion)
                    } else {
                      //Else (if the answer isnt Yes then its No)...
                        setTimeout(openMenu, 1000);
                        //reopen the main menu to the user
                    }
                });
            });

        } else {
          //Else (if the anser isn't Basic it had to be Cloze)
            inquirer.prompt([
                {
                    type: "input",
              ...
              //once we have the users cloze data run this function

                var cardObj = {
                  //builds and object from the text and cloze info
                    type: "ClozeCard",
                    text: cardData.text,
                    cloze: cardData.cloze
                };
                if (cardObj.text.indexOf(cardObj.cloze) !== -1) {
                  //checking to make sure the Cloze matches some text in the statement
                  ...
                  //if the cloze doesnt match then give a message to the user.
                ...
                inquirer.prompt([
                  ...
                  //once the user gives answer....
                    if (appData.anotherCard === "Yes") {
                      ...
                      //Else (if the answer isnt Yes then its No)...
                        setTimeout(openMenu, 1000);
                      //return the user to the open menu
                    }
                });
            });
        }
...
function askQuestions() {
    ...
    //playedCard stores the question from the card with index equal to the current counter.
        inquirer.prompt([
          //inquirer used to ask the question from the playedCard.
            {
                type: "input",
                message: playedCard,
                name: "question"
          ...
          //once the user answers
            ...
            //It will stop when counter=library.length
        });
    } else {
      	count=0;
...
function shuffleDeck() {
  ...
  //copy the flashcards into a new array
  for (var i = library.length - 1; i > 0; i--) {
    //this algorithm (Fisher-Yates shuffle) should jumble up the order of the copied array
...
function randomCard() {
  ...
  // get a random index number within the length of the current library

  playedCard = getQuestion(library[randomNumber]);
  //playedCard stores the question from the card with index equal to the randomNumber.
        inquirer.prompt([
          //inquirer used to ask the question from the playedCard.
            {
                type: "input",
